@extends('layouts.app')
  @section('content')
    ...
    <div class="col-sm-12" id="formulario">
      <div class="row">
        <div class="col-md-4">
          {!! Form::open(['route' => 'zonas/search', 'method' => 'post', 'novalidate']) !!}
        ...
        <div class="col-md-4 col-md-offset-4">
          @can('zonas.create')
            <a href="{{ route('zonas.create') }}" class="btn btn-success pull-right"><i class="fa fa-plus" aria-hidden="true"></i> Agregar</a>
          @endcan
        </div>
      </div>
      <table>
        ...
        <tbody>
          @foreach($zonas as $Zona)
            <tr>
              <td>{{$Zona->nombreZon}}</td>
              <td>
                @if($Zona->nombreMun != null)
                  <a href="{{ route('zonasmunicios', $Zona->id) }}" class="btn"><i class="fa fa-eye" aria-hidden="true"></i> Ver Municipios</a>
                @endif
                @if($Zona->nombreMun == null)
                  No tiene Municipios asociados
                @endif
              </td>
              @can('zonas.edit')
                <td class="espacio2" width="10px">
                  <a href="{{ route('zonas.edit', $Zona->id) }}" class="btn" data-toggle="tooltip" data-placement="right" data-original-title="Editar"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                </td>
              @endcan
              <!--@can('zonas.destroy')
...
<style>
    ...
    (min-device-width: 768px) and (max-device-width: 1024px)  {
      ...
      */
      ...
      td:nth-of-type(2):before { content: "Municipio"; }
