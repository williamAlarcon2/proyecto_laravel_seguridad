@extends('layouts.app')
  @section('content')
    <h4 class="with-border">Paises</h4>
    <div class="col-sm-12" id="formulario">
      <div class="row">
        <div class="col-md-4">
          {!! Form::open(['route' => 'paises/search', 'method' => 'post', 'novalidate']) !!}
        ...
        <div class="col-md-4 col-md-offset-4">
          @can('paises.create')
            <a href="{{ route('paises.create') }}" class="btn btn-success pull-right"><i class="fa fa-plus" aria-hidden="true"></i> Agregar</a>
          @endcan
        </div>
      </div>
      <table>
        ...
        <tbody>
          @foreach($paises as $Pais)
            <tr>
              <td>{{$Pais->nombrePai}}</td>
              @can('paises.edit')
                <td class="espacio2" width="10px">
                  <a href="{{ route('paises.edit', $Pais->id) }}" class="btn" data-toggle="tooltip" data-placement="right" data-original-title="Editar"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                </td>
              @endcan
              @can('paises.destroy')
